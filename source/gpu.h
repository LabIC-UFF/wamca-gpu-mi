 ...
 */
#define GPU_BIT2WORD(n, t) \
  (((n) / (sizeof(t) * CHAR_BIT)) + (((n) % (sizeof(t) * CHAR_BIT)) != 0))
/*!
...
#define GPU_BLOCK_SIZE(sz) \
  ((((sz) / GPU_ALIGN_SIZE) + ((sz) % GPU_ALIGN_SIZE != 0)) * GPU_ALIGN_SIZE)
#define GPU_BLOCK_CEIL(t, sz) GPU_BLOCK_SIZE(sizeof(t) * (sz))
#define GPU_BLOCK_CAST(v, p, sz) (v) = (typeof(*v)*)(p)
#define GPU_BLOCK_NEXT(v, p, sz) \
  (v) = ((typeof(*v)*)(((byte*)(p)) + GPU_BLOCK_CEIL(*p, sz)))
...
#define gpuHostMalloc(p, s, f) \
  GPU_CHECK_ERROR(__cudaHostMalloc((void**)(p), s, f))
#define gpuHostFree(p, f) GPU_CHECK_ERROR(__cudaHostFree(p, f))

#define gpuOccupancyMaxPotentialBlockSize(g, b, f, l) \
  cudaOccupancyMaxPotentialBlockSize(g, b, f, l)
#define gpuOccupancyMaxPotentialBlockSizeVariableSMem(g, b, f, s, l) \
  cudaOccupancyMaxPotentialBlockSizeVariableSMem(g, b, f, s, l)
...
#define gpuHostMalloc(p, s, f) \
  __cudaHostMalloc((void**)(p), s, cudaHostAllocPaged)
...
#define gpuMemcpyAsync(d, s, sz, k, f) \
  GPU_CHECK_ERROR(cudaMemcpyAsync(d, s, sz, k, f))
#define gpuGetDeviceCount(c) GPU_CHECK_ERROR(cudaGetDeviceCount(c))
#define gpuGetDeviceProperties(p, d) \
  GPU_CHECK_ERROR(cudaGetDeviceProperties(p, d))
...
#define gpuEventElapsedTime(m, s, e) \
  GPU_CHECK_ERROR(cudaEventElapsedTime(m, s, e))
...
#define gpuStreamAddCallback(s, c, p, f) \
  GPU_CHECK_ERROR(cudaStreamAddCallback(s, c, p, f))
 ...
 *
 * The flags parameter enables different options to be specified that affect the
 * allocation, as follows.
 *
 * cudaHostAllocPaged           This flags allocates paged-memory, just like
 * malloc().
 *
 * cudaHostAllocDefault         This flag's value is defined to be 0 and causes
 * cudaHostAlloc() to emulate cudaMallocHost(). cudaHostAllocPortable        The
 * memory returned by this call will be considered as pinned memory by all CUDA
 * contexts, not just the one that performed the allocation. cudaHostAllocMapped
 * Maps the allocation into the CUDA address space. The device pointer to the
 * memory may be obtained by calling cudaHostGetDevicePointer().
 * cudaHostAllocWriteCombined   Allocates the memory as write-combined (WC). WC
 * memory can be transferred across the PCI Express bus more quickly on some
 * system configurations, but cannot be read efficiently by most CPUs. WC memory
 * is a good option for buffers that will be written by the CPU and read by the
 ...
 */
inline cudaError_t __cudaHostMalloc(void** p, size_t size, int flags) {
  cudaError_t error;
#ifndef GPU_CUDA_DISABLED
 ...
 *
 * The flags parameter enables different options to be specified that affect the
 * memory releasing. If \a flag equal to \a cudaHostAllocPagged, function
 * peforms just like free(), otherwise performs like cudaFreeHost().
 *
 * @param   p       Device pointer to allocated memory
 *
 * @return  Returns error code.
 */
inline cudaError_t __cudaHostFree(void* p, int flags) {
  cudaError_t error;
#ifndef GPU_CUDA_DISABLED
  if (flags == cudaHostAllocPaged) {
    free(p);
...
#define gpuMalloc(p, s)                                                       \
  {                                                                           \
    ...
    checkCudaErrors(cudaMalloc((void**)(p), s));                              \
    fprintf(stderr, "*p=%p>\n", *p);                                          \
  }
#define gpuFree(p)                                                    \
  {                                                                   \
...
#define gpuHostMalloc(p, s, f)                                        \
  {                                                                   \
    int gid;                                                          \
    cudaGetDevice(&gid);                                              \
    fprintf(stderr, "%-15.15s: GPU%d call gpuHostMalloc(%p,%d,%d) <", \
            __FUNCTION__, gid, p, s, f);                              \
    checkCudaErrors(__cudaHostMalloc((void**)(p), s, f));             \
    fprintf(stderr, "*p=%p>\n", *p);                                  \
  }
#define gpuHostFree(p, f)                                                      \
  {                                                                            \
    int gid;                                                                   \
    cudaGetDevice(&gid);                                                       \
    fprintf(stderr, "%-15.15s: GPU%d call gpuHostFree(%p,%d) <", __FUNCTION__, \
            gid, p, f);                                                        \
    checkCudaErrors(__cudaHostFree(p, f));                                     \
    fprintf(stderr, ">\n");                                                    \
  }
