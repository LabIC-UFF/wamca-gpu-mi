// Copyright Igor Machado Coelho & Eyder Rios, 2016
// MIT License
...
class WAMCAExperiment {
 public:
  ...
  void kernelInit() {
    ...
    /*
            ...
            for (int kid = 0; kid < MLP_MAX_NEIGHBOR; kid++) {
                switch (kid) {
                case MLMI_SWAP:
                    tkernels[tkernelCount++] = new MLKernelSwap(problem, true);
       // true = TOTAL break; case MLMI_2OPT: tkernels[tkernelCount++] = new
       MLKernel2Opt(problem, true); break; case MLMI_OROPT1:
                    tkernels[tkernelCount++] = new MLKernelOrOpt(problem, 1,
       true); break; case MLMI_OROPT2: tkernels[tkernelCount++] = new
       MLKernelOrOpt(problem, 2, true); break; case MLMI_OROPT3:
                    tkernels[tkernelCount++] = new MLKernelOrOpt(problem, 3,
       true); break; default: EXCEPTION("Invalid move id: %d", kid);
                }
            }
    */
  }
  ...
  void initDevice() {
    ...
    for (int i = 0; i < kernelCount; i++) kernels[i]->init(true);

    //        lprintf("GPU%u, initializing %d tkernels\n", gpuId, tkernelCount);
    for (int i = 0; i < tkernelCount; i++) tkernels[i]->init(true);
  }
  ...
  void runGpuGpu(MLKernel* kernel, int& impr2, int& countImpr2, int& imprMoves2,
    ...
    /// countImpr2, movesCount);

    // printf("CHECK (impr=%d impr2=%d) (count=%d count2=%d) (imprMoves=%d
    // imprMoves2=%d)\n", impr, impr2, countImpr, countImpr2, imprMoves,
    // imprMoves2);
  }
  ...
  void runKernel(int k, std::vector<MLMove>* moves = NULL) {
    ...
    /// kernel, kernel->name, kernel->isTotal);
    ...
    int64_t curtime1 = tv1.tv_sec * 1000000 + tv1.tv_usec;

    // printf("%ld \t time \t search k=\t%d \t receiving value =
    // \t%d\tnada1=\t0\tnada2=\t0\tnada=\t0\n",curtime1, kMin, valor1);
    ...
    // assert((impr2 == impr) && (countImpr == countImpr2) &&
    // (imprMoves
    //== imprMoves2));
  ...
  unsigned int runWAMCA2016(int mMax = 3, int kMin = 0, int kMax = 3,
                            int* solution = NULL, unsigned int solutionSize = 0,
                            std::vector<MLMove>* moves = NULL,
    ...
    for (uint m = 0; m < mMax; m++) {
      ...
      /*
       ...
       tkernel, tkernel->name, tkernel->isTotal);
       ...
       impr += move.cost;
       countImpr++;
       //l4printf("tApply %s(%d,%d) = %d\n", tkernel->name, move.i, move.j,
       move.cost); tkernel->applyMove(move);
       }
       }
       duration = (( std::clock() - start ) / (double) CLOCKS_PER_SEC) * 1000;
...
#pragma omp parallel for
      ...
      // printf("%ld \t time \t search k=\t%d \t returning value =
      // \t%d\timprov_moves=\t%d\timprov_moves2=\t%d\timprov_real=\t%d\n",curtime,
      // kMin, valor,imprMoves, imprMoves2, valor-valor1); assert(valor1-valor
      // == imprMoves); // TODO: corrigir erro de movimento!
    }

    // TODO: APLICAR MOVIMENTOS DIRETAMENTE NA GPU! SE FOREM INDEPENDENTES FICA
    // FACIL :) EXISTE VANTAGEM EM FAZER ISSO DENTRO DE UM KERNEL COM
    // PARALELISMO DINÂMICO? USAR FUNCAO DE COMPRESSAO DA CUDAPP COMO
    // CONTABILIZAR NUMERO DE MOVIMENTOS UTEIS?
