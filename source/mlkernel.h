...
public:
    ...
    */
   void
   launchSleepKernel(ullong sleep = 100000);
   /*!
    * Launch test kernel.
    */
   void
    ...
    */
   void
   launchShowDataKernel(uint width, uint max = 0);
   /*!
    ...
    */
   void
   launchChecksumKernel(uint max = 0, bool show = false);
   /*!
   ...
   inline
   void
   setSolution(MLSolution *sol, bool ads = true) {
   ...
   inline
   void
   getSolution(MLSolution *sol, bool ads = true) {
   ...
   inline
   void
   addCallback(cudaStreamCallback_t func) {
    ...
    */
   int
   mergeGreedy(MLMove64 *merge, int &count);
   /*!
...
private:
    MLKernel *kernel;
public:
    MLKernelSharedSize(MLKernel *krn) {
        kernel  = krn;
    }
    int
    operator() (int blockSize) {
