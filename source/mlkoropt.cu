 ...
 */
__global__ void kernelOrOpt(const MLADSData* gm_ads, MLMovePack* gm_move,
                            const int k, const int size) {
  /*!
  ...
  i = by + k + 1;
  gm_data = ADS_COST_PTR(gm_ads, sm_rsize) + sm_rsize * i;
...
__global__ void kernelOrOptTotal(const MLADSData* gm_ads, MLMovePack* gm_move,
                                 const int k, const int size) {
  /*!
   * Shared memory variables
   */
...
__global__ void kernelOrOpt(const MLADSData* gm_ads, MLMovePack* gm_move,
  ...
  __shared__ int *sm_soldist,  // Solution distances
  ...
  for (c = 0; (c < cmax) && ((ctx = c * blockDim.x + tx) < size); c++) {
    ...
    if (ctx < size - sm_tour - k - 1) {
      if (ctx >= by) {
        ...
        cost += (size - j - k) *
                (int(GPU_DIST_COORD(i + k - 1, j + k)) - sm_soldist[j + k]);
        cost += (size - j) * (int(GPU_DIST_COORD(j + k - 1, i))) -
                (size - i - k) * sm_soldist[i + k];

        s = 0;
        for (l = i + 1; l < i + k; l++) s += sm_soldist[l];
        cost += (i - j) * s;

        s = 0;
        for (l = i + k + 1; l < j + k; l++) s += sm_soldist[l];
        cost += k * s;
      } else {
        //
        // i > j
        ...
        cost += (size - i - k) *
                (int(GPU_DIST_COORD(i - 1, i + k)) - sm_soldist[i + k]);
        ...
        s = 0;
        for (l = j + 1; l < i; l++) s += sm_soldist[l];
        cost -= k * s;
      }
...
void MLKernelOrOpt::defineKernelGrid() {
  int gsize, bsize;
   ...
   */
  grid.x = 1;
  grid.y = solSize + !problem.costTour - tag -
           2;  ////solSize - problem.params.costTour - tag; // TODO: FIX!!
  grid.z = 1;
  ...
  l4printf("Kernel %s\tgrid(%d,%d,%d)\tblck(%d,%d,%d)\tshared=%u (%u KB)\n",
           name, grid.x, grid.y, grid.z, block.x, block.y, block.z, shared,
           shared / 1024);
}

void MLKernelOrOpt::launchKernel() {
  ...
  // Calls kernel
  //    if(!isTotal)
  // std::cout << "MLKernelOrOpt" << tag << "::launchKernel - grid(x: " <<
  // grid.x << ", y: " << grid.y << ", z: " << grid.z
  //		<< ") block(x: " << block.x << ", y: " << block.y << ", z: " <<
  // block.z << ") shared: " << shared << std::endl;
...
void MLKernelOrOpt::applyMove(MLMove& move) {
  uint i, j, l;
  ushort temp[tag];
  ...
  for (; i <= j /*&& i < solution->clientCount*/; i++) {
    solution->weights[i] =
        problem.clients[solution->clients[i - 1]].weight[solution->clients[i]];
  }
