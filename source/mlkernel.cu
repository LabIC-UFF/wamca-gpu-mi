...
#define def_canSwapMerge(m1, m2)                                         \
  ((GPU_ABS(int(m1.i - m2.i)) > 1) && (GPU_ABS(int(m1.i - m2.j)) > 1) && \
   (GPU_ABS(int(m1.j - m2.i)) > 1) && (GPU_ABS(int(m1.j - m2.j)) > 1))

// __forceinline__
__forceinline__ __device__ bool f_canSwapMerge(MLMove64& m1, MLMove64& m2) {
  return (GPU_ABS(int(m1.i - m2.i)) > 1) && (GPU_ABS(int(m1.i - m2.j)) > 1) &&
         (GPU_ABS(int(m1.j - m2.i)) > 1) && (GPU_ABS(int(m1.j - m2.j)) > 1);
}

__forceinline__ __device__ bool f_can2OptMerge(MLMove64& m1, MLMove64& m2) {
  return (m1.j < m2.i - 1) || (m1.i > m2.j + 1) || (m2.j < m1.i - 1) ||
         (m2.i > m1.j + 1);
}

__forceinline__ __device__ bool f_canOrOptMerge(MLMove64& m1, MLMove64& m2) {
  // register
  int k1, min1, max1, k2, min2, max2;
...
__global__ void testSwap(MLMove64* g_move64, int numElems) {
  if (tx >= numElems) return;
  extern __shared__ MLMove64 s_move64[];

  // register
  MLMove64 m = g_move64[tx];
  // if(m >= 0){
  ...
  for (int i = 0; i <= tx; ++i) {  // tx 'or' numElems (?) symmetry?
    if (i != tx) {
      // register
      MLMove64 mi = s_move64[i];
      if (mi.cost < 0) {
        // register
        MLMove64 mx = s_move64[tx];
        if (!f_canSwapMerge(mi, mx)) s_move64[tx].cost = 0;
      }
    }
    __syncthreads();
  }

  if (s_move64[tx].cost == 0) g_move64[tx] = s_move64[tx];
  ...
  for (int i = 0; i <= tx; ++i) {  // tx 'or' numElems (?) symmetry?
    if (i != tx) {
      ...
      if (mi.cost < 0) {
        // register
        MLMove64 mx = s_move64[tx];
        if (!f_can2OptMerge(mi, mx)) s_move64[tx].cost = 0;
      }
    }
    __syncthreads();
  }
  ...
  for (int i = 0; i <= tx; ++i) {  // tx 'or' numElems (?) symmetry?
    if (i != tx) {
      ...
      if (mi.cost < 0) {
        // register
        MLMove64 mx = s_move64[tx];
        if (!f_canOrOptMerge(mi, mx)) s_move64[tx].cost = 0;
      }
    }
    __syncthreads();
  }
...
__global__ void testSwapTotal(MLMove64* g_move64, int numElems) {
  ...
  for (int i = 0; i <= numElems; ++i) {  // tx 'or' numElems (?) symmetry?
    // register
    MLMove64 mi = g_move64[i];
    if (mi.cost < 0) {
      for (c = 0; (c < cmax) && ((ctx = c * blockDim.x + tx) < numElems); ++c)
        if (ctx > i) {
          // register
          MLMove64 mx = g_move64[ctx];
          if (!f_canSwapMerge(mi, mx)) g_move64[ctx].cost = 0;
        }
    }
    __syncthreads();
  }
    ...
    if (mi.cost < 0) {
      for (c = 0; (c < cmax) && ((ctx = c * blockDim.x + tx) < numElems); ++c)
        if (ctx > i) {
          // register
          MLMove64 mx = g_move64[ctx];
          if (!f_can2OptMerge(mi, mx)) g_move64[ctx].cost = 0;
        }
    }
    __syncthreads();
  }
    ...
    if (mi.cost < 0) {
      for (c = 0; (c < cmax) && ((ctx = c * blockDim.x + tx) < numElems); ++c)
        if (ctx > i) {
          // register
          MLMove64 mx = g_move64[ctx];
          if (!f_canOrOptMerge(mi, mx)) g_move64[ctx].cost = 0;
        }
    }
    __syncthreads();
  }
...
void MLKernel::mergeGPU() {
  thrust::device_ptr<MLMovePack> td_moveData(moveData);
  thrust::sort(td_moveData, td_moveData + moveElems, OBICmp());

  //       gpuMemcpyAsync(transBuffer.p_void,moveData,moveElems *
  //       sizeof(MLMove64),cudaMemcpyDeviceToHost,stream); MLMove64* p_pack =
  //       transBuffer.p_move64; sync(); printf("FIRST PRINT!\n"); int count =
  //       0; for(unsigned i=0; i<moveElems; i++) {
  ...
  switch (this->id) {
    case MLMI_SWAP:
      gpuOccupancyMaxPotentialBlockSizeVariableSMem(
          &minGridSize, &blockSize, testSwap,
          __cudaOccupancyB2DHelper(sMemSize), blockSizeLimit);
      ...
      if (!isTotal)
        testSwap<<<grid, block, sMemSize, stream>>>((MLMove64*)moveData,
                                                    moveElems);
      else {
        block.x = ::min(1024, moveElems);
        sMemSize = 0;  // no shared memory
        testSwapTotal<<<grid, block, sMemSize, stream>>>((MLMove64*)moveData,
                                                         moveElems);
      }
      break;
    case MLMI_2OPT:
      gpuOccupancyMaxPotentialBlockSizeVariableSMem(
          &minGridSize, &blockSize, test2Opt,
          __cudaOccupancyB2DHelper(sMemSize), blockSizeLimit);
      // printf("minGridSize=%d blockSize=%d moveElems=%d\n",minGridSize,
      // blockSize, moveElems);
      ...
      if (!isTotal)
        test2Opt<<<grid, block, sMemSize, stream>>>((MLMove64*)moveData,
                                                    moveElems);
      else {
        block.x = ::min(1024, moveElems);
        sMemSize = 0;  // no shared memory
        test2OptTotal<<<grid, block, sMemSize, stream>>>((MLMove64*)moveData,
                                                         moveElems);
      }
      break;
    case MLMI_OROPT1:
    ...
    case MLMI_OROPT29:
      gpuOccupancyMaxPotentialBlockSizeVariableSMem(
          &minGridSize, &blockSize, testOrOpt,
          __cudaOccupancyB2DHelper(sMemSize), blockSizeLimit);
      // printf("minGridSize=%d blockSize=%d moveElems=%d\n",minGridSize,
      // blockSize, moveElems);
      ...
      if (!isTotal)
        testOrOpt<<<grid, block, sMemSize, stream>>>((MLMove64*)moveData,
                                                     moveElems);
      else {
        block.x = ::min(1024, moveElems);
        sMemSize = 0;  // no shared memory
        testOrOptTotal<<<grid, block, sMemSize, stream>>>((MLMove64*)moveData,
                                                          moveElems);
      }
      break;
    default:
...
void MLKernel::init(bool solCreate) {
  ...
  //     * +---------+----------------+----------------+----------------+----------------+-----+----------------+
  //     * | b elems | size elems |gap| size elems |gap| size elems |gap| size
  //     elems |gap| ... | size elems |gap|
  //     * +---------+----------------+----------------+----------------+----------------+-----+----------------+
  //     *
  //     * b = MLP_ADSINFO_ELEMS = GPU_GLOBAL_ALIGN/sizeof(uint) = 128/4 = 32
  //     elems
  //     *
  ...
  // else
  //     adsDataSize = ADS_INFO_SIZE +            3  * adsRowElems *
  //     sizeof(uint);
  ...
  gpuHostMalloc(&transBuffer.p_void, transBufferSize,
                0);  //???gpuTask.params.allocFlags); TODO: fix
...
void MLKernel::term() {
  ...
  gpuHostFree(transBuffer.p_void,
              0);  //????gpuTask.params.allocFlags); TODO: fix

  if (solDestroy && solution) delete solution;

  reset();
}

int MLKernel::bestMove(MLMove& move) {
  const MLMove64* moves;
  ullong time;
  uint i, b;
  ...
  for (i = 1; i < moveElems; i++) {
    ...
    if (moves[i].cost < moves[b].cost) b = i;
  }
...
inline bool canSwapMerge(MLMove64* m1, MLMove64* m2)  // TODO: SWAP MERGE!
{
  switch (m2->id) {
    case MLMI_SWAP:
      //    	return true;

      return (abs(int(m1->i - m2->i)) > 1) && (abs(int(m1->i - m2->j)) > 1) &&
             (abs(int(m1->j - m2->i)) > 1) && (abs(int(m1->j - m2->j)) > 1);
    ...
    default:
      ...
      MINMAX(m2->i, m2->j, min2, max2);
      k2 = MLMI_OROPT_K(m2->id);
      return (m1->j < min2 - 1) || (m1->i > max2 + k2) ||
             ((m1->i < min2 - 1) && (m1->j > max2 + k2));
  }
  // return false;
}

inline bool can2OptMerge(MLMove64* m1, MLMove64* m2) {
  switch (m2->id) {
    case MLMI_SWAP:
      return canSwapMerge(m2, m1);
    case MLMI_2OPT:
      return (m1->j < m2->i - 1) || (m1->i > m2->j + 1) ||
             (m2->j < m1->i - 1) || (m2->i > m1->j + 1);
    default:
      int k2, min2, max2;
...
inline bool canOrOptMerge(MLMove64* m1, MLMove64* m2) {
  switch (m2->id) {
    case MLMI_SWAP:
      return canSwapMerge(m2, m1);
    case MLMI_2OPT:
    ...
    default:
      ...
      //            lprintf("(%d,%d) x (%d,%d)\n",m1->i,m1->j,m2->i,m2->j);
      //            lprintf("\t[ max(%d,%d) + %d < min(%d,%d) ] ||
      //            ",m1->i,m1->j,k1,m2->i,m2->j); lprintf("\t[ min(%d,%d) >
      //            max(%d,%d) + %d ] =\n",m1->i,m1->j,m2->i,m2->j,k2);
      //            lprintf("\t[ %d < %d ] || [ %d > %d ] = %d\n",max1 +
      //            k1,min2,min1,max2 + k2,
...
bool canMerge(MLMove64* move1, MLMove64* move2) {
  switch (move1->id) {
    case MLMI_SWAP:
...
bool compMove(MLMove64 m1, MLMove64 m2) { return m1.cost < m2.cost; }

int MLKernel::mergeGreedy(MLMove64* merge, int& count) {
  MLMove64* moves;
  ullong time;
  int i, j, n;
  int cost;
...
// #define MLP_MERGE_SPLIT
  ...
  j = n - 1;
  while (i < j) {
    while ((i < n) && (moves[i].cost < 0)) i++;
    while ((j >= 0) && (moves[j].cost >= 0)) j--;
    if (i < j) swap(moves[i], moves[j]);
  }
  n = i;
#else
  n = moveElems;
  ...
  if (isTotal) {
    ...
    if (n > 120000) {
      ...
      printf(
          "WARNING: THE GPU-GPU WILL PROBABLY RETURN A DIFFERENT (MORE "
          "PRECISE) VALUE THAN THIS GPU-CPU\n");
    }
  }
  ...
  for (i = 0; i < graphMerge.vertexCount; i++) {
    ...
    //    graphMerge[i]->i, graphMerge[i]->j, graphMerge[i]->cost);
...
//     * Generate movement conflict graph (Graphviz DOT file)
  ...
  for (i = 0; i < graphMerge.vertexCount; i++) {
    if (graphMerge.getFlag(i)) continue;

    merge[count] = *graphMerge[i];
    cost += graphMerge[i]->cost;
    // lprintf("choose %s(%d,%d) = %d\n", this->name, graphMerge[i]->i,
    // graphMerge[i]->j, graphMerge[i]->cost);
    ...
    for (j = i + 1; j < graphMerge.vertexCount; j++) {
      if (graphMerge.hasEdge(i, j)) graphMerge.setFlag(j, 1);
    }
  }
